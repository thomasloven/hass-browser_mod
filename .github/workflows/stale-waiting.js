module.exports = async ({ github, context }) => {
  ...
  // Helper to check if a user has write access or is the repository owner
  const hasWriteAccess = async (association, username) => {
    if (!username) return false;
    if (username === OWNER_USERNAME) return true;
    if (association && ['OWNER', 'COLLABORATOR', 'MEMBER'].includes(association)) {
      return true;
    }
    // Check if the user is a collaborator as a fallback (especially for commits where association is not provided)
    try {
  ...
  for (const issue of issues) {
    ...
    // 4. Filter comments made by users without write access (ignore maintainer activity)
    const nonMaintainerComments = [];
    for (const c of postLabelComments) {
      const hasWrite = await hasWriteAccess(c.author_association, c.user ? c.user.login : '');
      if (!hasWrite) {
        nonMaintainerComments.push(c);
      }
    }
    ...
    if (isPullRequest) {
      ...
      const nonMaintainerReviewComments = [];
      for (const c of postLabelReviewComments) {
        const hasWrite = await hasWriteAccess(c.author_association, c.user ? c.user.login : '');
        if (!hasWrite) {
          nonMaintainerReviewComments.push(c);
        }
      }
      ...
      const nonMaintainerPostLabelReviews = [];
      for (const r of postLabelReviews) {
        const hasWrite = await hasWriteAccess(r.author_association, r.user ? r.user.login : '');
        if (!hasWrite) {
          nonMaintainerPostLabelReviews.push(r);
        }
      }
      ...
      const postLabelCommits = commits.filter(c => {
        const commitDate = getCommitDate(c);
        return commitDate && commitDate > labelAddedAt;
      });
      const nonMaintainerPostLabelCommits = [];
      for (const c of postLabelCommits) {
        const login = getCommitAuthorLogin(c);
        const hasWrite = await hasWriteAccess(null, login);
        if (!hasWrite) {
          nonMaintainerPostLabelCommits.push(c);
        }
      }
    ...
    const allNonMaintainerActivity = [
      ...nonMaintainerComments,
      ...nonMaintainerReviews,
      ...nonMaintainerCommits
    ];

    if (allNonMaintainerActivity.length > 0) {
    ...
    if (daysElapsed > DAYS_BEFORE_STALE) {
      if (!hasStaleLabel) {
        ...
        try {
          ...
          const staleMessage = `There hasn't been any activity on this ${itemType} recently. This ${itemType} has been automatically marked as stale because we are waiting for a response and will be closed if no further activity occurs within ${DAYS_BEFORE_CLOSE} days.\n\nThank you for your contributions.`;
        ...
        if (staleLabelEvents.length > 0) {
          ...
          const staleDaysElapsed = staleMsElapsed / MS_IN_DAY;
          ...
          if (staleDaysElapsed > DAYS_BEFORE_CLOSE) {
            ...
            try {
              ...
              const itemType = isPullRequest ? 'pull request' : 'issue';
              const closeMessage = `This ${itemType} has been closed because we did not receive a response.`;
