...
|`adaptive`| Enable the popup to use an adaptive dialog. A regular popup dialog will be used on larger screens while a bottom sheet will be used on smaller devices. NOTE: `classic` style is not compatible with adaptive dialog as it will never be used.|
